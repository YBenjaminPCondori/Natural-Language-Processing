...
\caption{Overall experimental pipeline from project audit and split verification through cell-by-cell preprocessing, feature extraction, baseline modelling, transformer tuning, final test evaluation, and error analysis.}
...
The coursework code includes dataset loading, cell-by-cell preprocessing, feature extraction, data audit checks, model training, validation-based hyperparameter tuning, final test evaluation, prediction saving, and visualisation. Random seeds were fixed where possible for Python, NumPy, PyTorch, and CUDA. The final results can be reproduced by running the staged notebooks in order, beginning with the preprocessing and EDA notebook and then the classical TF-IDF notebook, or by running the combined main experimental notebook after the staged flow has been checked. The pipeline loads LEDGAR from Hugging Face, applies the top-20-label filtering pipeline, records the preprocessing techniques in \texttt{outputs/preprocessing\_techniques.md}, trains the baseline and supervised models, supports W\&B random and Bayesian sweeps for the main transformer when enabled, evaluates the selected model once on the held-out test set, and saves the result tables and figures used in this report.
