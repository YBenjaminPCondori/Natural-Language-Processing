...
\caption{Overall experimental pipeline from project audit and split verification through baseline modelling, transformer tuning, final test evaluation, and error analysis.}
...
For classical models, clause text is converted into TF-IDF vectors using unigram and bigram features. For transformer models, clause text is tokenised using the relevant Hugging Face tokenizer with padding and truncation. The maximum sequence length is treated as a tunable transformer hyperparameter because clause lengths vary across the dataset.
...
For classical models, random search or a compact grid search is used because TF-IDF classifiers are relatively fast and have a small search space. For the main transformer model, a two-stage Weights \& Biases Sweeps strategy is used.

\subsection{Stage 5A: Wide Random Search}

The first sweep uses random search to explore a broad transformer hyperparameter space. This stage is designed to identify promising regions rather than produce the final model.
...
The second sweep uses Bayesian optimisation to refine the best region found during random search. This is more suitable than a large grid search for BERT-style models because the search space contains interacting continuous hyperparameters such as learning rate, weight decay, and warmup ratio.
...
After tuning, the best configuration is saved to \texttt{outputs/best\_hyperparameters.json}, and all sweep runs are summarised in \texttt{outputs/sweep\_results.csv}. Failed or skipped trials, including out-of-memory failures, are logged rather than silently ignored.
...
\midrule
Stage 5A & Random Search & 20 & \todo{value} \\
Stage 5B & Bayesian Optimisation & 15 & \todo{value} \\
\bottomrule
\end{tabular}
...
\caption{Validation macro-F1 across hyperparameter tuning trials. The figure compares broad random search with narrowed Bayesian optimisation.}
...
The coursework code includes dataset loading, preprocessing, data audit checks, model training, validation-based hyperparameter tuning, final test evaluation, prediction saving, and visualisation. Random seeds were fixed where possible for Python, NumPy, PyTorch, and CUDA. The final results can be reproduced by running the main experimental notebook, which loads LEDGAR from Hugging Face, applies the top-20-label filtering pipeline, trains the baseline and supervised models, performs W\&B random and Bayesian sweeps for the main transformer, evaluates the selected model once on the held-out test set, and saves the result tables and figures used in this report.
