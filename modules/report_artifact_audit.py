...
REQUIRED_ARTIFACTS: tuple[RequiredArtifact, ...] = (
    ...
    RequiredArtifact(
        ...
        "Dataset clause-length figure used by report.tex",
        "modules.coursework_artifacts.generate_dataset_verification or modules.report_exports.export_report_figures",
        ("figures/text_length_distribution.png", "results/eda/clause_length_histogram.png"),
    ),
    RequiredArtifact(
        "figures/model_comparison_macro_f1.png",
        "Main model comparison figure used by report.tex",
        "modules.coursework_artifacts.generate_comparison_figures or modules.report_exports.export_report_figures",
        ("results/final_model_comparison.png", "outputs/figures/model_comparison_macro_f1.png"),
    ),
    RequiredArtifact(
        "figures/qwen_invalid_predictions.png",
        "Qwen invalid-output figure used by report.tex",
        "modules.report_exports.plot_qwen_invalid_predictions or modules.coursework_artifacts.generate_qwen_invalid_figure",
        ("outputs/figures/qwen_invalid_predictions.png",),
    ),
    RequiredArtifact(
        "figures/confusion_matrix_best_model.png",
        "Best supervised model confusion matrix figure used by report.tex",
        "Final report export stage should copy the selected best-model confusion matrix to this canonical filename.",
        ("outputs/figures/confusion_matrix_best_model.png", "results/transformer/confusion_matrices/distilbert_base_uncased_confusion_matrix.png"),
    ),
    RequiredArtifact(
        "figures/agentic_review_workflow.png",
        "Confidence-review workflow figure used by report.tex",
        "modules.report_exports.plot_agentic_workflow or modules.coursework_artifacts.generate_agentic_review_artifacts",
        ("outputs/figures/agentic_review_workflow.png",),
    ),
    RequiredArtifact(
        "outputs/main_results.csv",
        "Report main-results table source",
        "modules.report_exports.export_main_results or modules.coursework_artifacts.canonical_results",
    ...
    RequiredArtifact(
        ...
        "HPT summary values for Table 5 in report.tex",
        "modules.report_exports.export_hpt_report_aliases or modules.coursework_artifacts.generate_tuning_artifacts",
        ("outputs/sweep_results.csv",),
    ),
    RequiredArtifact(
        "outputs/preprocessing_techniques.md",
        "Preprocessing rundown referenced by report.tex",
        "modules.preprocessing.write_preprocessing_rundown",
    ),
    RequiredArtifact(
        "outputs/metrics/final_model_comparison_summary.csv",
        "Combined LEDGAR-test and CUAD-external summary referenced by report.tex",
        "modules.cuad_external.build_final_model_comparison_summary or report artifact adapter",
        ("outputs/main_results.csv",),
    ),
)
...
def audit_hpt(project_root: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    rows = [
        ...
        row(
            "Stage 5A random-search runner exists",
            "PASS" if "stage5a_random_trial" in hpt_text else "FAIL",
            "Report describes Stage 5A as random search with optional W&B logging.",
            "Keep Stage 5A named and exported as stage5a_random_trial.",
        ),
        row(
            "Stage 5A default is 8 trials",
            "PASS" if "random_trials: int = 8" in hpt_text else "FAIL",
            "report.tex states Stage 5A is configurable with default 8.",
            "Set TransformerHPTConfig.random_trials to 8 or update report.tex.",
        ),
        row(
            "Stage 5A uses 2 epochs per trial",
            "PASS" if re.search(r'"?epochs"?\s*:\s*\[2\]', hpt_text) or '"epochs": [2]' in hpt_text else "FAIL",
            "report.tex states Stage 5A uses 2 epochs per trial.",
            "Keep DEFAULT_STAGE5A_SEARCH_SPACE['epochs'] fixed to [2].",
        ),
        row(
        ...
        row(
            "Stage 5B Bayesian/Optuna runner exists",
            "PASS" if "stage5b_bayes_trial" in hpt_text and "optuna" in hpt_text.lower() else "FAIL",
            "report.tex describes Bayesian optimisation; the implementation uses Optuna with optional W&B logging.",
            "Keep Stage 5B exported as stage5b_bayes_trial and document Optuna-based Bayesian optimisation.",
        ),
        row(
            "Stage 5B default is 8 trials",
            "PASS" if "bayes_trials: int = 8" in hpt_text else "FAIL",
            "report.tex states Stage 5B is configurable with default 8.",
            "Set TransformerHPTConfig.bayes_trials to 8 or update report.tex.",
        ),
        row(
            "Stage 5B uses 3 epochs per trial",
            "PASS" if re.search(r'"?epochs"?\s*:\s*\[3\]', hpt_text) or '"epochs": [3]' in hpt_text else "FAIL",
            "report.tex states Stage 5B uses 3 epochs per trial.",
            "Keep DEFAULT_STAGE5B_SEARCH_SPACE['epochs'] fixed to [3].",
        ),
        row(
            "Stage 5B uses narrowed search space",
            "PASS" if "DEFAULT_STAGE5B_SEARCH_SPACE" in hpt_text and "stage5b_search_space" in hpt_text else "FAIL",
            "report.tex describes a narrowed Bayesian search space.",
            "Use DEFAULT_STAGE5B_SEARCH_SPACE for Stage 5B.",
        ),
        row(
        ...
        row(
            ...
            "PASS" if "final_retrain_epochs: int = 4" in hpt_text and "num_train_epochs=1 if config.smoke_test else int(config.final_retrain_epochs)" in hpt_text else "FAIL",
            "Appendix A states the final transformer uses 4 epochs after validation selection.",
            "Keep final_retrain_epochs at 4 for non-smoke final retrain.",
        ),
        row(
        ...
        row(
            ...
            "Run when training is intended: python scripts/run_transformer_hpt.py --model-name distilbert-base-uncased --random-trials 8 --bayes-trials 8 --wandb",
        ),
        row(
...
def audit_evaluation(project_root: Path) -> list[dict[str, Any]]:
    ...
    add(
        ...
        "PASS" if evidence_exists(project_root, "figures/confusion_matrix_best_model.png") else "FAIL",
        "figures/confusion_matrix_best_model.png",
        "Copy or regenerate best-model confusion matrix as figures/confusion_matrix_best_model.png.",
    )
