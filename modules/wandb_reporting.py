...
def start_wandb_run(
    *,
    ...
    if not _has_wandb_credentials(mode):
        ...
        print("Run wandb.login() or set WANDB_API_KEY, then rerun the notebook to send logs.")
        return None
...
def _log_csv_table(run: Any, path: Path, *, max_rows: int) -> bool:
    try:
        ...
        table_name = f"tables/{_safe_key(path.stem)}"
...
def _artifact_files(
    paths: ProjectPaths,
    *,
    log_text_tables: bool,
    log_model_files: bool,
) -> list[Path]:
    root = paths.project_root
    files = _existing_files(root, SAFE_CSV_ARTIFACTS + SAFE_JSON_ARTIFACTS)
    files.extend(path for path in (root / "outputs" / "figures").glob("*.png") if path.is_file())
    files.extend(path for path in (root / "figures").glob("*.png") if path.is_file())
    files.extend(path for path in (root / "results").glob("**/*.png") if path.is_file())
    files.extend(path for path in (root / "results").glob("**/classification_reports/*.json") if path.is_file())
